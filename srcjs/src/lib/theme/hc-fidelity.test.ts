...
describe("HC encoding fidelity tokens", () => {
  it("standard mode → caret is empty string", () => {
    const r = resolveTheme(createWire({ brand: "#0099CC", mode: "standard" }, "t"));
    expect(r.cssVars["--tv-hc-caret-char"]).toBe("");
  });

  it("high-contrast mode → caret is ▸", () => {
    const r = resolveTheme(createWire({ brand: "#0099CC", mode: "high-contrast" }, "t"));
    expect(r.cssVars["--tv-hc-caret-char"]).toBe("▸");
  });
  ...
  it("ring width is mode-invariant 1.5px", () => {
    const std = resolveTheme(createWire({ brand: "#0099CC", mode: "standard" }, "t"));
    const hc  = resolveTheme(createWire({ brand: "#0099CC", mode: "high-contrast" }, "t"));
