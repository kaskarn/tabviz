...
function resolveRampStepRef(refStr: string, ramps: TokenRampsV3): string | null {
...
/**
 * Resolve a ColorRefV3 (or plain string, or null) to a hex string.
 *
 * Accepted forms:
...
export function resolveRef(
  refOrHex: ColorRefV3 | string | null | undefined,
  ramps: TokenRampsV3,
): string | null {
  ...
  if (typeof refOrHex === "string") {
    ...
    return resolveToken(refOrHex as TokenNameV3, ramps);
  }
  if ("hex" in refOrHex) {
  ...
  const baseHex = rampStepHex !== null
    ? rampStepHex
    : resolveToken(refStr as TokenNameV3, ramps);
...
export function defaultRoles(): ThemeRolesV3 {
  return {
    emphasis: {
      fg: ref("ink"),
      markerFill: ref("ink"),
...
export function defaultClusters(): ClustersV3 {
  return {
    header: {
...
/**
 * Build a fully-resolved WebThemeV3 from inputs.
 *
 ...
 *
 * @param inputs T1 inputs (`ThemeInputsV3`)
