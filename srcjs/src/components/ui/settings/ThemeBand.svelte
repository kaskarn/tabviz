<!--
  ThemeBand — the THEME half of the rebuilt settings panel
  (settings-overhaul P2). Everything here is Tier-1 / variant: it edits
  `authoringInputs` and re-resolves through the cascade. This band's
  edits TRAVEL WITH THE THEME (vs FigureBand's per-figure state).

  IA (locked blueprint): IDENTITY (anchor rows + status disclosure) ·
  SURFACE (shell / header / series / border preset / texture) · TYPE
  (family / base / scale) · then collapsed COLOR SYSTEM / EFFECTS /
  GEOMETRY with value-chip summaries. Disclosure depth ≤ 1 (law).
...
<script lang="ts">
  ...
  import { AnchorRow, EnumRow } from "$components/theme-controls";
  ...
  function statusTriple(key: StatusKey): OklchTriple {
    ...
    return hex ? (hexToOklch(hex) ?? { L: 0.6, C: 0.1, H: 0 }) : { L: 0.6, C: 0.1, H: 0 };
  }
  ...
  const statusSummary = $derived.by(() => {
    const n = inputs ? Object.keys(inputs.status ?? {}).length : 0;
  ...
  let colorSystemOpen = $state(false);
