...
{#if inputs}
  <div class="identity-tab">
    ...
    <DisclosureField label="Geometry" summary={geometrySummary} bind:open={geometryOpen}>
      ...
      <Field label="Rule regular"><Slider value={inputs.geometry?.border_width?.regular ?? DEFAULT_BORDER_WIDTH.regular} min={0} max={5} step={0.25} suffix="px" ariaLabel="Border width regular" onchange={(v) => writeGeometry(preview, "border_width", "regular", v)} oncommit={(v) => writeGeometry(commit, "border_width", "regular", v)} /></Field>
