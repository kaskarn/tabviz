...
# WebTheme2. PR 10 deletes v1 and renames WebTheme2 -> WebTheme.

# Hex regex shared across v2 validators. Renamed in PR 10 when v1 retires.
hex_pattern_v2 <- "^#([0-9A-Fa-f]{3}|[0-9A-Fa-f]{6}|[0-9A-Fa-f]{8})$"

# Validator factory: each named slot must be NA-or-hex.
make_color_validator_v2 <- function(slots) {
  function(self) {
    invalid <- character()
    for (s in slots) {
      v <- S7::prop(self, s)
      if (!is.na(v) && !grepl(hex_pattern_v2, v)) {
...
ThemeInputs <- new_class(
  ...
  validator = function(self) {
    ...
    bad <- self@neutral[!grepl(hex_pattern_v2, self@neutral)]
    if (length(bad) > 0L) {
      invalid <- c(invalid, paste("neutral has invalid:", paste(bad, collapse = ", ")))
    }
    bad <- self@series_anchors[!grepl(hex_pattern_v2, self@series_anchors)]
    if (length(bad) > 0L) {
      invalid <- c(invalid,
...
Surfaces <- new_class(
  ...
  validator = make_color_validator_v2(c("base", "muted", "raised"))
)
...
Content <- new_class(
  ...
  validator = make_color_validator_v2(c("primary", "secondary", "muted", "inverse"))
)
...
Dividers <- new_class(
  ...
  validator = make_color_validator_v2(c("subtle", "strong"))
)
...
AccentRoles <- new_class(
  ...
  validator = make_color_validator_v2(
...
StatusColors <- new_class(
  ...
  validator = make_color_validator_v2(c("positive", "negative", "warning", "info"))
)
...
TextRole <- new_class(
  ...
  validator = function(self) {
    ...
    if (!is.na(self@fg) && !grepl(hex_pattern_v2, self@fg)) {
...
HeaderVariant <- new_class(
  ...
  validator = make_color_validator_v2(c("bg", "fg", "rule"))
)
...
RowState <- new_class(
  ...
  validator = make_color_validator_v2(c("bg", "fg"))
)
...
CellCluster <- new_class(
  ...
  validator = make_color_validator_v2(c("bg", "fg", "border"))
)
...
#' WebTheme2: 3-tier theme specification (v2).
#'
