...
test_that("row_bold accepts formula expression", {
  ...
  spec <- web_spec(df, label = "study", row_bold = ~ pval < 0.05)
...
test_that("row_emphasis accepts formula expression", {
  ...
  spec <- web_spec(df, label = "study", row_emphasis = ~ pval < 0.05)
...
test_that("row styling still accepts column names (backwards compatibility)", {
  ...
  spec <- web_spec(df, label = "study", row_bold = "is_sig")
...
test_that("invalid formula in row styling produces error", {
  ...
  expect_error(
    web_spec(df, label = "study", row_bold = ~ nonexistent_column < 0.05),
    "nonexistent_column"
  )
})
...
test_that("row styling validates missing column name", {
  ...
  expect_error(
    web_spec(df, label = "study", row_bold = "nonexistent"),
    "nonexistent"
  )
})
...
test_that("formula returning length-1 value is recycled", {
  ...
  spec <- web_spec(df, label = "study", row_bold = ~ TRUE)
