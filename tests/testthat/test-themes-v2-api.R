test_that("web_theme_v2_custom builds from inputs", {
  t <- web_theme_v2_custom(
    name = "navy",
    inputs = list(
      brand = "#1F3A5F",
      accent = "#B08938",
  ...
  expect_true(inherits(t, "tabviz::WebTheme2"))
...
test_that("web_theme_v2_custom inherits from base_theme", {
  t <- web_theme_v2_custom(
    name = "lancet-italic-tweak",
    base_theme = web_theme_lancet_v2(),
...
test_that("web_theme_v2_custom rejects unknown property names", {
  expect_error(
    web_theme_v2_custom(inputs = list(zonk = "#000")),
    "Unknown property"
  )
})

test_that("set_inputs updates Tier 1 fields and re-resolves", {
  t <- web_theme_default_v2()
...
test_that("set_theme_field rejects empty path", {
  expect_error(set_theme_field(web_theme_default_v2(), character(), "x"),
               "at least one element")
})

test_that("set_spacing_v2 overrides one token, density preset fills rest", {
  t <- web_theme_default_v2()
  t2 <- set_spacing_v2(t, row_height = 99)
  expect_equal(t2@spacing@row_height, 99)
  expect_equal(t2@spacing@header_height, t@spacing@header_height)
})

test_that("modifiers reject v1 themes", {
  expect_error(set_inputs(web_theme_default(), brand = "#000"),    "WebTheme2")
  expect_error(set_variants(web_theme_default(), density = "compact"), "WebTheme2")
  expect_error(set_theme_field(web_theme_default(), c("inputs", "brand"), "#000"),
               "WebTheme2")
  expect_error(set_spacing_v2(web_theme_default(), row_height = 24), "WebTheme2")
})
