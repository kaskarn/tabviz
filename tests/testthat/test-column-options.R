...
test_that("col_stars accepts domain and serializes as numeric vector", {
  col <- col_stars("score", domain = c(0, 100))
  expect_equal(col@options$stars$domain, c(0, 100))
})

test_that("col_stars rejects malformed domain", {
  expect_error(col_stars("score", domain = c(100, 0)), "sorted")
  expect_error(col_stars("score", domain = 5), "length")
  expect_error(col_stars("score", domain = c(NA, 1)))
})

test_that("col_stars without domain has null domain", {
...
test_that("new column options round-trip through web_spec", {
  ...
  spec <- web_spec(
    ...
    columns = list(
      ...
      col_stars("rating", max_stars = 5, domain = c(0, 100)),
      col_bar("w", scale = "log")
    )
  )
