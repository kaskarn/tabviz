...
`)}function Et(t,r,n){var f,l;const o=[],i=n.spacing.padding;let e=r.footerY;if((f=t.labels)!=null&&f.caption){const c=v(n.typography.fontSizeSm);o.push(`<text x="${i}" y="${e}"
      font-family="${n.typography.fontFamily}"
      font-size="${c}px"
...
`)}function it(t,r,n,o,i,e,f,l){const c=[],h=v(e.typography.fontSizeSm),a=e.typography.fontWeightMedium,s=e.typography.fontWeightBold,p=K(t),g=I.DEFAULT_COLUMN_WIDTH,u=l??I.DEFAULT_LABEL_WIDTH,N=($,_)=>$+_/2+h*G.TEXT_BASELINE_ADJUSTMENT;if(p){const $=i/2,_=i/2;let x=n;f&&(c.push(`<text x="${x+w.TEXT_PADDING}" y="${N(o,i)}"
        font-family="${e.typography.fontFamily}"
        font-size="${h}px"
        font-weight="${a}"
        fill="${e.colors.foreground}">${D(f)}</text>`),c.push(`<line x1="${x+u}" x2="${x+u}"
        y1="${o}" y2="${o+i}"
        stroke="${e.colors.border}" stroke-width="1" opacity="0.3"/>`),x+=u);for(const d of t)if(d.isGroup){const T=rt(d,g),E=x+T/2;c.push(`<text x="${E}" y="${N(o,$)}"
          font-family="${e.typography.fontFamily}"
          font-size="${h}px"
          font-weight="${s}"
          text-anchor="middle"
...
`)}function _t(t,r,n,o,i,e,f){const l=[],c=v(f.typography.fontSizeBase),h=o+i/2+c*G.TEXT_BASELINE_ADJUSTMENT,a=r*w.INDENT_PER_LEVEL;return l.push(`<rect x="${n}" y="${o}"
    width="${e}" height="${i}"
    fill="${f.colors.primary}" opacity="${ut}"/>`),l.push(`<text x="${n+w.TEXT_PADDING+a}" y="${h}"
    font-family="${f.typography.fontFamily}"
    font-size="${c}px"
    font-weight="${f.typography.fontWeightBold}"
    fill="${f.colors.foreground}">${D(t)}</text>`),l.join(`
`)}function st(t,r){const n=new Map;for(const o of r)if(o.type==="bar"){let i=0;for(const e of t){const f=e.metadata[o.field];typeof f=="number"&&f>i&&(i=f)}n.set(o.field,i>0?i:1)}return n}function at(t,r,n,o,i,e,f,l=I.DEFAULT_LABEL_WIDTH,c=0,h){var u,N,$,_,x,d,T,E,y,b,A,H,L;const a=[],s=v(e.typography.fontSizeBase),p=o+i/2+s*G.TEXT_BASELINE_ADJUSTMENT;let g=n;if(f){const m=c*w.INDENT_PER_LEVEL+(((u=t.style)==null?void 0:u.indent)??0)*w.INDENT_PER_LEVEL,M=(N=t.style)!=null&&N.bold?e.typography.fontWeightBold:e.typography.fontWeightNormal,U=($=t.style)!=null&&$.italic?"italic":"normal",R=l-m-w.TEXT_PADDING*2,O=q(t.label,R,s,0);if(a.push(`<text x="${g+w.TEXT_PADDING+m}" y="${p}"
      font-family="${e.typography.fontFamily}"
      font-size="${s}px"
      font-weight="${M}"
      font-style="${U}"
      fill="${((_=t.style)==null?void 0:_.color)??e.colors.foreground}">${D(O)}</text>`),(x=t.style)!=null&&x.badge){const S=String(t.style.badge),F=s*.8,X=4,W=F+X*2,P=t.label.length*s*.55,B=g+w.TEXT_PADDING+m+P+6,k=S.length*F*.6+X*2,C=o+(i-W)/2;a.push(`<rect x="${B}" y="${C}" width="${k}" height="${W}"
        rx="3" fill="${e.colors.primary}" opacity="0.15"/>`),a.push(`<text x="${B+k/2}" y="${C+W/2+F*.35}"
        text-anchor="middle"
        font-family="${e.typography.fontFamily}"
        font-size="${F}px"
        font-weight="500"
        fill="${e.colors.primary}">${D(S)}</text>`)}g+=l}for(const m of r){const M=typeof m.width=="number"?m.width:I.DEFAULT_COLUMN_WIDTH,U=bt(t,m),{textX:R,anchor:O}=J(g,M,m.align);if(m.type==="bar"&&typeof t.metadata[m.field]=="number"){const S=t.metadata[m.field],F=h==null?void 0:h.get(m.field),X=((T=(d=m.options)==null?void 0:d.bar)==null?void 0:T.maxValue)??F??100,W=((y=(E=m.options)==null?void 0:E.bar)==null?void 0:y.color)??e.colors.primary,P=e.shapes.pointSize*2,Y=M-w.TEXT_PADDING*2-50,k=Math.min(S/X*Y,Y);a.push(`<rect x="${g+w.TEXT_PADDING}" y="${o+i/2-P/2}"
        width="${Math.max(0,k)}" height="${P}"
        fill="${W}" opacity="0.7" rx="2"/>`);const C=g+M-w.TEXT_PADDING;a.push(`<text x="${C}" y="${p}"
        font-family="${e.typography.fontFamily}"
        font-size="${s}px"
        text-anchor="end"
        fill="${e.colors.foreground}">${Z(S)}</text>`)}else if(m.type==="sparkline"&&Array.isArray(t.metadata[m.field])){let S=t.metadata[m.field];Array.isArray(S[0])&&(S=S[0]);const F=((A=(b=m.options)==null?void 0:b.sparkline)==null?void 0:A.height)??16,X=((L=(H=m.options)==null?void 0:H.sparkline)==null?void 0:L.color)??e.colors.primary,W=w.TEXT_PADDING*2,P=Ht(S,g+w.TEXT_PADDING,o+i/2-F/2,M-W,F);a.push(`<path d="${P}" fill="none" stroke="${X}" stroke-width="1.5"/>`)}else a.push(`<text x="${R}" y="${p}"
        font-family="${e.typography.fontFamily}"
        font-size="${s}px"
        text-anchor="${O}"
        fill="${e.colors.foreground}">${D(U)}</text>`);g+=M}return a.join(`
`)}function bt(t,r){var o,i,e,f;if(r.type==="interval")return gt(t.point,t.lower,t.upper);if(r.type==="numeric"){const l=t.metadata[r.field];return typeof l=="number"?Z(l):""}if(r.type==="pvalue"){const l=t.metadata[r.field];if(typeof l!="number")return"";const c=(i=(o=r.options)==null?void 0:o.pvalue)==null?void 0:i.stars,h=((f=(e=r.options)==null?void 0:e.pvalue)==null?void 0:f.thresholds)??[.05,.01,.001];let a="";return c&&(l<h[2]?a="***":l<h[1]?a="**":l<h[0]&&(a="*")),l<.001?`<0.001${a}`:`${l.toFixed(3)}${a}`}const n=t.metadata[r.field];return n!=null?String(n):""}function Ht(t,r,n,o,i){if(t.length===0)return"";const e=Math.min(...t),l=Math.max(...t)-e||1;return`M${t.map((h,a)=>{const s=r+a/(t.length-1)*o,p=n+i-(h-e)/l*i;return`${s.toFixed(1)},${p.toFixed(1)}`}).join("L")}`}const lt=6;function Q(t,r,n){const o=t.metadata[r];return o!=null&&typeof o=="number"&&!Number.isNaN(o)?o:n}function It(t,r){return r<=1?0:-((r-1)*lt)/2+t*lt}function At(t,r,n,o,i,e=[]){var x;let f;e.length===0?f=[{point:t.point,lower:t.lower,upper:t.upper,color:null}]:f=e.map(d=>({point:Q(t,d.pointCol,t.point),lower:Q(t,d.lowerCol,t.lower),upper:Q(t,d.upperCol,t.upper),color:d.color??null}));const l=f.filter(d=>d.point!=null&&!Number.isNaN(d.point)&&d.lower!=null&&!Number.isNaN(d.lower)&&d.upper!=null&&!Number.isNaN(d.upper));if(l.length===0)return"";const c=o.shapes.pointSize,h=t.metadata.weight;let a=c;if(h){const d=.5+Math.sqrt(h/100)*1.5;a=Math.min(Math.max(c*d,3),c*2.5)}const s=o.shapes.lineWidth,p=o.colors.intervalLine,g=w.WHISKER_HALF_HEIGHT,u=((x=t.style)==null?void 0:x.type)==="summary",$=o.shapes.summaryHeight/2,_=[];return l.forEach((d,T)=>{const E=r+It(T,l.length),y=n(d.lower),b=n(d.upper),A=n(d.point),H=d.color??p,L=d.color??(d.point>i?o.colors.intervalPositive:d.point<i?o.colors.intervalNegative:o.colors.muted);if(u){const m=[`${y},${E}`,`${A},${E-$}`,`${b},${E}`,`${A},${E+$}`].join(" ");_.push(`
        ...
        </g>`)}),_.join("")}function wt(t,r,n,o,i,e,f,l){const h=l.shapes.summaryHeight/2,a=i(r),s=i(t),p=i(n),g=e+Math.max(0,Math.min(f,a)),u=e+Math.max(0,Math.min(f,s)),N=e+Math.max(0,Math.min(f,p));return`<polygon points="${[`${g},${o}`,`${u},${o-h}`,`${N},${o}`,`${u},${o+h}`].join(" ")}"
    fill="${l.colors.summaryFill}"
    stroke="${l.colors.summaryBorder}"
    stroke-width="1"/>`}function Nt(t,r,n,o,i){const e=[],f=typeof n.axis.tickCount=="number"?n.axis.tickCount:w.DEFAULT_TICK_COUNT,l=Array.isArray(n.axis.tickValues)?n.axis.tickValues:t.ticks(f),c=v(n.typography.fontSizeSm);e.push(`<line x1="${i}" x2="${i+r.forestWidth}"
    y1="0" y2="0" stroke="${n.colors.border}" stroke-width="1"/>`);for(const h of l){const a=i+t(h);e.push(`<line x1="${a}" x2="${a}" y1="0" y2="4"
      ...
      font-family="${n.typography.fontFamily}"
      font-size="${c}px"
      fill="${n.colors.secondary}">${yt(h)}</text>`)}return o&&e.push(`<text x="${i+r.forestWidth/2}" y="28"
      text-anchor="middle"
      font-family="${n.typography.fontFamily}"
      font-size="${c}px"
      font-weight="500"
...
`)}</g>`}function ct(t,r,n,o,i,e){let l=`<line x1="${t}" x2="${t}" y1="${r}" y2="${n}"
    stroke="${i}" stroke-width="1" stroke-dasharray="${o==="dashed"?"6,4":o==="dotted"?"2,2":"none"}"/>`;return e&&(l+=`<text x="${t}" y="${r-4}" text-anchor="middle"
      font-size="10px" fill="${i}">${D(e)}</text>`),l}class z extends Error{constructor(r){super(`SVG Generator: ${r}`),this.name="SVGGeneratorError"}}function Dt(t){if(!t||typeof t!="object")throw new z("Invalid spec: expected an object");const r=t;if(!r.data||typeof r.data!="object")throw new z("Invalid spec: missing or invalid 'data' property");const n=r.data;if(!Array.isArray(n.rows))throw new z("Invalid spec: 'data.rows' must be an array");if(!r.theme||typeof r.theme!="object")throw new z("Invalid spec: missing or invalid 'theme' property");const o=r.theme;if(!o.colors||typeof o.colors!="object")throw new z("Invalid spec: missing or invalid 'theme.colors'");if(!o.typography||typeof o.typography!="object")throw new z("Invalid spec: missing or invalid 'theme.typography'");if(!o.spacing||typeof o.spacing!="object")throw new z("Invalid spec: missing or invalid 'theme.spacing'")}function Lt(t,r={}){Dt(t);const n=t.theme,o=pt(t,r),i=n.spacing.padding,e=Array.isArray(t.columns)?t.columns:[],f=j(e,"left"),l=j(e,"right"),c=V(e,"left"),h=V(e,"right"),a=I.DEFAULT_LABEL_WIDTH+c.reduce((y,b)=>y+(typeof b.width=="number"?b.width:I.DEFAULT_COLUMN_WIDTH),0),s=i+a,p=Tt(t,o.forestWidth),g=s+o.forestWidth,u=[];u.push(`<?xml version="1.0" encoding="UTF-8"?>
<svg xmlns="http://www.w3.org/2000/svg"
  width="${o.totalWidth}" height="${o.totalHeight}"
  viewBox="0 0 ${o.totalWidth} ${o.totalHeight}">`);const N=r.backgroundColor??n.colors.background;u.push(`<rect width="100%" height="100%" fill="${N}"/>`),u.push(mt(t,o,n));const $=o.mainY;u.push(it(f,c,i,$,o.headerHeight,n,t.data.labelHeader??"Study",I.DEFAULT_LABEL_WIDTH)),h.length>0&&u.push(it(l,h,g,$,o.headerHeight,n)),u.push(`<line x1="${i}" x2="${o.totalWidth-i}"
    y1="${$+o.headerHeight}" y2="${$+o.headerHeight}"
    stroke="${n.colors.border}" stroke-width="1"/>`);const _=et(t);if(t.data.includeForest&&o.forestWidth>0){const y=o.mainY+o.headerHeight,b=s+p(t.data.nullValue);u.push(ct(b,y,y+o.plotHeight,"dashed",n.colors.muted));const A=Array.isArray(t.annotations)?t.annotations:[];for(const H of A)if(H.type==="reference_line"){const L=s+p(H.x);u.push(ct(L,y,y+o.plotHeight,H.style,H.color??n.colors.accent,H.label))}if(_.forEach((H,L)=>{if(H.type==="data"){const m=y+L*o.rowHeight+o.rowHeight/2;u.push(At(H.row,m,M=>s+p(M),n,t.data.nullValue,t.data.effects))}}),t.data.overall&&o.showOverallSummary&&typeof t.data.overall.point=="number"&&!Number.isNaN(t.data.overall.point)&&typeof t.data.overall.lower=="number"&&!Number.isNaN(t.data.overall.lower)&&typeof t.data.overall.upper=="number"&&!Number.isNaN(t.data.overall.upper)){const H=y+o.summaryYPosition;u.push(wt(t.data.overall.point,t.data.overall.lower,t.data.overall.upper,H,p,s,o.forestWidth,n))}u.push(Nt(p,o,n,t.data.axisLabel,s))}const x=o.mainY+o.headerHeight,d=t.data.rows,T=st(d,c),E=st(d,h);return _.forEach((y,b)=>{const A=x+b*o.rowHeight;if(y.type==="group_header")u.push(_t(y.label,y.depth,i,A,o.rowHeight,o.totalWidth-i*2,n));else{const H=y.row,L=y.depth,m=dt(L),M=b%2===1?ht:0,U=Math.max(m,M);U>0&&u.push(`<rect x="${i}" y="${A}"
          width="${o.totalWidth-i*2}" height="${o.rowHeight}"
          fill="${n.colors.muted}" opacity="${U}"/>`),u.push(at(H,c,i,A,o.rowHeight,n,!0,I.DEFAULT_LABEL_WIDTH,L,T)),h.length>0&&u.push(at(H,h,g,A,o.rowHeight,n,!1,0,L,E))}u.push(`<line x1="${i}" x2="${o.totalWidth-i}"
      y1="${A+o.rowHeight}" y2="${A+o.rowHeight}"
      stroke="${n.colors.border}" stroke-width="1" opacity="0.5"/>`)}),u.push(Et(t,o,n)),u.push("</svg>"),u.join(`
`)}function Mt(t,r){const n=JSON.parse(t);return Lt(n,r)}globalThis.generateSVG=Mt})();
