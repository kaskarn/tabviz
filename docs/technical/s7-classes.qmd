...
| `GroupHeaderStyles` | Per-level row-group header styling |

:::{.callout-important}
## `ColorPalette` slot names vs `set_colors()` arguments
The S7 slots on `ColorPalette` are still named:

- `interval` — default CI marker fill
- `interval_positive`, `interval_negative`, `interval_neutral` — direction-coded marker fills
- `interval_line` — CI line color

These slot names are unchanged. The user-facing arguments to `set_colors()` are the renamed forms:

- `ci_marker_fill` (writes to slot `interval`)
- `ci_marker_positive` / `ci_marker_negative` / `ci_marker_neutral`
- `ci_line` (writes to slot `interval_line`)

The old `interval*` argument names on `set_colors()` still work but emit a deprecation warning. If you read or write the slots directly via `theme@colors@interval_positive`, use the slot names. If you call `set_colors()`, use the `ci_marker_*` / `ci_line` argument names.
:::
