...
### `tabviz()` with `col_forest()` - Forest Plot
...
| `col_forest()` | Forest plot column | `col_forest(point = "hr", lower = "lo", upper = "hi")` |
...
| `col_bar()` | Horizontal bars | `col_bar("weight")` |
...
| `col_group()` | Group columns | `col_group("Results", col_interval(point = "hr", lower = "lo", upper = "hi"), col_pvalue("p"))` |
...
tabviz(data, label = "study", columns = list(col_forest(...)), split_by = "region")

# Hierarchical split
tabviz(data, label = "study", columns = list(col_forest(...)), split_by = c("region", "country"), shared_axis = TRUE)
```
...
col_forest(point = "md", lower = "lo", upper = "hi", scale = "linear", null_value = 0)
