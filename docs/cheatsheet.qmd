...
Quick reference for webforest functions and arguments.
...
| `col_interval()` | Point (CI) format | `col_interval("HR (95% CI)")` |
...
```r
forest_plot(..., theme = web_theme_default() |>
...
forest_plot(data, ..., split_by = "region")

# Hierarchical split
forest_plot(data, ..., split_by = c("region", "country"), shared_axis = TRUE)
```
...
# Column groups
col_group("Treatment", col_n("n_tx"), col_events("ev_tx", "n_tx"), position = "left")
