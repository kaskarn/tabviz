...
| `viz_bar()` | Bar charts with axis | `viz_bar(viz_bar_effect("value"))` |
| `viz_boxplot()` | Box-and-whisker plots | `viz_boxplot(viz_boxplot_effect(data = "values"))` |
| `viz_violin()` | Violin/density plots | `viz_violin(viz_violin_effect(data = "values"))` |
...
# Bar chart
viz_bar(
  viz_bar_effect("value", color = "#3b82f6"),
  axis_range = c(0, 100)
)

# Boxplot from array data
viz_boxplot(
  viz_boxplot_effect(data = "measurements", color = "#8b5cf6"),
  show_outliers = TRUE
)

# Boxplot from pre-computed stats
viz_boxplot(
  viz_boxplot_effect(min = "min", q1 = "q1", median = "median", q3 = "q3", max = "max")
)

# Violin plot
viz_violin(
  viz_violin_effect(data = "scores", color = "#f59e0b"),
