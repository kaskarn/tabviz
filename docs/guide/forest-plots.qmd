...
This chapter covers adding forest plot columns to your tables using `col_forest()`. For quick forest plots, you can also use the `forest_plot()` convenience wrapper.
...
This chapter covers: data mapping, scales, multiple effects, markers, axis control, interactivity, and exporting.

## Data Mapping

tabviz uses a **column-mapping pattern**: you specify which columns contain your data, not the values themselves.
...
tabviz(data, label = "study", columns = list(
  col_forest(point = "my_hr", lower = "ci_lo", upper = "ci_hi",
             scale = "log", null_value = 1)
))
```
...
tabviz(structured, label = "label", row_type = "rtype", row_bold = "rbold",
  columns = list(
...
tabviz(marker_data, label = "study", title = "Marker Styling with Formulas",
...
The `null_value` parameter draws a dashed vertical reference line. Add custom reference lines with `forest_refline()` inside `col_forest()`:

```{r}
tabviz(data, label = "study", title = "With Custom Reference Line",
  columns = list(
    col_forest(point = "my_hr", lower = "ci_lo", upper = "ci_hi",
               scale = "log", null_value = 1,
               annotations = list(
...
Add multiple reference lines with custom styling:

```{r}
tabviz(data, label = "study", title = "Clinical Thresholds",
  columns = list(
    col_forest(point = "my_hr", lower = "ci_lo", upper = "ci_hi",
               scale = "log", null_value = 1,
               annotations = list(
...
tabviz(tooltip_data, label = "study",
...
tabviz(forest_data, label = "study", row_type = "rtype", row_bold = "rbold",
  columns = list(
