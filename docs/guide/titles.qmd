...
## Axis labels live on the visualization column

Axis labels (the text below a forest / bar / box / violin chart's x-axis) are a property of the **viz column**, not of the table. Set them inside the `viz_*()` constructor with `axis_label = "..."`:
...
See [Forest Plots — Axis Control](visualizations/forest-plots.qmd#axis-control) for the full set of axis options (`axis_range`, `axis_ticks`, `axis_gridlines`, `axis_label`).
...
- [Visualizations](visualizations/index.qmd) — Forest plots and other graphical columns
