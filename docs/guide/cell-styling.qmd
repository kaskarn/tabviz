...
This guide covers per-cell styling — formatting that applies to individual cells inside a specific column, with optional conditions on the cell's own value or other columns in the row.

::: {.callout-tip}
## Where this fits

This page covers the **Cell layer** of the styling cascade. For the overall picture (Theme → Row → Cell → Marker, in order of increasing specificity) see [Styling Overview](styling/index.qmd). For row-wide formatting and the semantic-token shortcuts (`row_accent`, `row_emphasis`, `row_muted`, `row_bold`, `row_fill`), see [Row Styling](row-styling.qmd).
:::

## Cell Style Parameters

Every `col_*()` function accepts these styling parameters. The first six are literal — you set the value directly. The last five are **theme-aware semantic tokens**: tag a cell with `accent` and the theme paints it in its own accent color, restyling automatically when you swap themes.

| Parameter | Type | Description |
|-----------|------|-------------|
| `bold` | Column name or formula | Bold text when TRUE |
| `italic` | Column name or formula | Italic text when TRUE |
| `color` | Column name or formula | Text color (literal CSS color) |
| `bg` | Column name or formula | Background color (literal CSS color) |
| `badge` | Column name or formula | Badge label |
| `icon` | Column name or formula | Emoji/unicode icon |
| `emphasis` *(semantic)* | Column name or formula | Bold + theme primary color |
| `muted` *(semantic)* | Column name or formula | Lighter color, reduced prominence |
| `accent` *(semantic)* | Column name or formula | Bold + theme accent color |
| `bold` *(semantic)* | Column name or formula | Weight bump only, no color change. Same name as the literal `bold` above; pass via the `style_bold` arg if you need to disambiguate. |
| `fill` *(semantic)* | Column name or formula | Bold + strong row-fill background (auto-contrasting fg) |

The literal `color` / `bg` win over semantic tokens on the same cell — they're more specific. See [Themes — Semantic tokens](themes.qmd#semantic-tokens) for the underlying `RowSemantic` bundles.
...
The pronoun rule depends on where you write the formula:

| Where | What `.x` means | Bare names available? |
|-------|-----------------|------------------------|
| Row level (`tabviz(row_bold = ~ ...)`) | Not used — write bare column names | Yes, e.g. `~ pval < 0.05` |
| Cell level (`col_*(bold = ~ ...)`) | The current column's values | Yes — bare names also resolve |

```{r}
#| eval: false
# Cell level: .x is this column's values
col_pvalue("pval", bold = ~ .x < 0.05)

# Cell level: you can also reference other columns by bare name
col_numeric("hr", color = ~ ifelse(pval < 0.05, "green", "gray"))

# Row level: always use bare column names
tabviz(data, ..., row_bold = ~ pval < 0.05)
```
...
tabviz(styled,
  label = "study",
  columns = list(
    col_interval("hr", "lower", "upper", color = "effect_color"),
    viz_forest(point = "hr", lower = "lower", upper = "upper",
               scale = "log", null_value = 1)
  )
)
...
tabviz(outcomes,
  ...
  columns = list(
    ...
    col_interval("hr", "lower", "upper", color = "text_color"),
    viz_forest(point = "hr", lower = "lower", upper = "upper",
               scale = "log", null_value = 1)
  )
)
...
- [Row Styling](row-styling.qmd) — Whole-row formatting and semantic-token shortcuts
- [Themes](themes.qmd) — Theme-aware color tokens (`accent`, `emphasis`, etc.)
- [Columns](columns.qmd) — `col_*()` argument reference
