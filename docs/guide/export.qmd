...
`save_plot()` operates in three sizing modes — see the [reference](../reference/save_plot.qmd#save_plot) for the full table. The short version: which args you pass determines the mode, not the values.

| Argument | Default | Purpose |
|---|---|---|
| `x` | required | A `WebSpec`, htmlwidget, or `SplitForest` |
| `file` | required | Output path; format inferred from extension (`.svg`, `.png`, `.pdf`) |
| `width` | `NULL` | Display width in pixels. `NULL` preserves natural; with `height` or `ratio` triggers relayout. |
| `height` | `NULL` | Display height in pixels. Same semantics as `width`. |
| `ratio` | `NULL` | Target aspect ratio (`width / height`). Mode-3 trigger. |
| `anchor` | `NULL` | When only `ratio` is given, picks `"width"` / `"height"` / `"auto"` for how dims fall out of it. |
| `flex` | `TRUE` | Cap on flex-column absorption (`TRUE` = 2× / 0.5×; `FALSE` = no flex; numeric = custom). |
| `auto_wrap` | `FALSE` | Bump `wrap` on long-text columns to fill tall-aspect height with content. |
| `scale` | `2` | Raster fidelity multiplier for PNG (warns on `.svg` / `.pdf`). |

**Natural mode** (no `width` / `height` / `ratio`) renders at the spec's intrinsic dimensions — exactly what `tabviz_natural_dimensions()` reports:

```{r}
#| eval: false
# Natural — let the spec decide its own shape
save_plot(p, "natural.svg")
```
...
## What matches the screen — and what deliberately doesn't {#fidelity}

Exports are WYSIWYG by contract: the SVG/PDF/PNG pipeline draws from the
same theme tokens, the same layout engine, and (for borders, headers,
axis typography) literally the same derivation functions as the live
widget. A small set of exceptions is deliberate:

* **Browser-only effects** — glass panes, glows, and aurora backdrop
  blobs need real compositing; exports omit them rather than fake them.
* **Interactive page state** — an export always renders the **whole
  table**, even when the live widget is showing one page of a paginated
  spec (a figure is the dataset; a page is a view). Multi-page PDF
  pagination is the exception below.
* **Raw-spec column widths** — driving `generateSVG` directly from
  hand-written JSON (no R, no browser) estimates text widths instead of
  measuring them; columns can land a few pixels off. Every supported
  flow avoids this: R's `save_plot()` injects pixel-exact `systemfonts`
  measurements, and widget-triggered exports reuse the live widths.

Anything else that differs between screen and export is a bug — the
regression suite diffs the two pixel-by-pixel across the preset matrix.
