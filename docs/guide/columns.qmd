...
tabviz(demo_data,
  ...
  columns = list(
    ...
    col_forest(point = "or", lower = "lower", upper = "upper", null_value = 1),
    col_pvalue("pval", header = "P-value", width = 80),
    col_interval(point = "or", lower = "lower", upper = "upper", header = "OR (95% CI)", width = 120)
  ),
...
Columns appear in the order they're listed. Place `col_forest()` wherever you want the forest plot:

```r
columns = list(
  col_numeric("n", "N"),           # Left of forest
  col_forest(point = "hr", lower = "lower", upper = "upper"),  # Forest plot in middle
  col_interval(point = "hr", lower = "lower", upper = "upper", header = "OR (95% CI)")  # Right of forest
)
```
...
A common layout convention for forest plots:

- **Left of forest**: Study metadata (sample size, events, weights)
- **Right of forest**: Results and effect estimates (intervals, p-values)
:::
...
```r
# Custom formatting
col_interval(point = "hr", lower = "lower", upper = "upper", header = "HR (95% CI)", decimals = 1, sep = ", ")
...
col_interval(point = "hr", lower = "lower", upper = "upper", imprecise_threshold = 10)
```
...
tabviz(outcomes_data,
  label = "study",
  columns = list(
    col_forest(point = "primary_hr", lower = "primary_lower", upper = "primary_upper", null_value = 1, scale = "log"),
    col_group("Primary Endpoint",
      col_interval(point = "primary_hr", lower = "primary_lower", upper = "primary_upper", header = "HR (95% CI)"),
