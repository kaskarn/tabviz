...
If you need a badge that stays with a specific column regardless of layout, use `set_column_style()` with `badge = "col"` on the target column instead of `set_row_style()`. Column-level badges are bound to their column and unaffected by row-level re-anchoring.

```{r}
#| eval: false
# Row-level: anchors to leftmost column (may move)
tabviz(data, ...) |>
  set_row_style(badge = "sig")

# Column-level: anchors to "study" column (stable)
web_spec(data, ...) |>
  set_column_style("study", badge = "sig") |>
  tabviz()
```
