...
::: {.callout-tip}
## Theme variations
The same data structure works with any theme. Try `web_theme_nejm()`, `web_theme_nejm()`, or `web_theme_aurora()` for different journal styles, or one of the LOTR presets (`web_theme_dwarven()` / `web_theme_dwarven()` / `web_theme_dwarven()`) for non-journal contexts. See [Feature Showcases](showcases.qmd#all-theme-variants) for a visual comparison of all seven built-in themes.
:::
