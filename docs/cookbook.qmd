...
## Build a Plot Step by Step (Fluent API)

Use the fluent API to separate spec creation from styling:
...
base_spec <- web_spec(step_data,
  point = "hr", lower = "lower", upper = "upper",
  label = "study", scale = "log", null_value = 1
)

# Step 2: Add styling via pipes
styled_spec <- base_spec |>
  set_marker_style(color = "sig_color", shape = "study_shape") |>
  set_row_style(bold = "is_summary")

# Step 3: Render
forest_plot(styled_spec, theme = web_theme_modern())
```

## Create Spec Variants from Base

Reuse a base spec with different styling:
...
v1 <- base |> set_marker_style(color = "sig_color")

# Variant 2: Shaped by study type
v2 <- base |> set_marker_style(shape = "study_shape")

# Variant 3: Both
v3 <- base |> set_marker_style(color = "sig_color", shape = "study_shape")

# Render each for comparison
forest_plot(v1)  # Colored
forest_plot(v2)  # Shaped
forest_plot(v3)  # Both
```
