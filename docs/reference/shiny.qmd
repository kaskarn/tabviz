...
| Argument | Description |
|----------|-------------|
| `expr` | Expression that returns a `split_forest()` result rendered with `forest_plot()` |
| `env` | Environment for evaluation |
| `quoted` | Whether expr is already quoted |
...
### split_forest_select {#split_forest_select}

Select a specific sub-plot in a split forest.

```r
split_forest_select(proxy, key)
```

| Argument | Description |
|----------|-------------|
| `proxy` | A `split_forest_proxy` object |
...
- [web_spec()](web_spec.qmd) for creating specifications
