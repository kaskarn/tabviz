...
All row styling arguments accept column names containing the styling values:

| Argument | Column Contains |
|----------|-----------------|
| `row_type` | Row type: `"header"`, `"data"`, `"summary"`, `"spacer"` |
| `row_bold` | `TRUE`/`FALSE` for bold text |
| `row_italic` | `TRUE`/`FALSE` for italic text |
| `row_color` | CSS color strings (e.g., `"#1a365d"`) |
| `row_bg` | Background color strings |
| `row_badge` | Badge text to display |
| `row_icon` | Emoji or unicode icons |
| `row_indent` | Indentation levels (0, 1, 2, ...) |
| `row_emphasis` | `TRUE`/`FALSE` for emphasis styling (bold + primary color) |
| `row_muted` | `TRUE`/`FALSE` for muted styling (lighter, reduced prominence) |
| `row_accent` | `TRUE`/`FALSE` for accent styling (theme accent color) |

### Marker Styling Arguments

| Argument | Column Contains |
|----------|-----------------|
| `marker_color` | CSS color strings for marker fill |
| `marker_shape` | Shape values: `"square"`, `"circle"`, `"diamond"`, `"triangle"` |
| `marker_opacity` | Opacity values from 0 to 1 |
| `marker_size` | Size multipliers (1 = default size) |
