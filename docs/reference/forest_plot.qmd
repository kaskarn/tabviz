...
Style entire rows based on column values. Each argument accepts a column name containing the styling values.

| Argument | Column Contains | Effect |
|----------|-----------------|--------|
| `row_type` | `"header"`, `"data"`, `"summary"`, `"spacer"` | Controls row behavior and marker display |
...
Control the appearance of interval markers (points and lines).

| Argument | Column Contains | Effect |
|----------|-----------------|--------|
| `marker_color` | CSS color strings | Marker fill color |
| `marker_shape` | `"square"`, `"circle"`, `"diamond"`, `"triangle"` | Marker shape |
| `marker_opacity` | 0–1 | Transparency |
| `marker_size` | Numeric multiplier | Size (1 = default, 2 = double) |
...
| Argument | Description |
|----------|-------------|
| `width_mode` | `"natural"` (centered at natural width) or `"fill"` (expand to container) |
| `height_preset` | `"full"`, `"small"` (300px), `"medium"` (500px), `"large"` (900px), `"container"` |
