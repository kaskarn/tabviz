...
6. **Deprecated arguments** — Did 0.9.0 rename what you're passing? See [Recent renames](#recent-renames).
...
The 0.9.0 release renamed several arguments. The old names still work for one cycle and emit `lifecycle::deprecate_warn()` — but if you're hitting a "no such argument" error, check this table:

| Old | New | Where |
|---|---|---|
| `col_interval(..., sep = " ")` | `separator = " "` | `col_interval()` |
| `col_reference(..., icon = TRUE)` | `show_icon = TRUE` | `col_reference()` |
| `col_n()`, `col_bar()`, `col_pvalue()`, `col_sparkline()` with no `field` | Pass `field` explicitly | `col_*()` |
| `set_colors()`, `set_typography()`, `set_shapes()`, `set_axis()`, `set_layout()`, `set_group_headers()`, `set_effect_colors()`, `set_marker_shapes()` | `set_inputs()`, `set_variants()`, `set_spacing()`, `set_theme_field(theme, path, value)` | v2 modifier surface (theme rewrite) |
| `web_interaction(show_filters = ...)` | `enable_filters = ...` | `web_interaction()` |
| `tabviz(axis_range=, axis_ticks=, axis_gridlines=)` | Pass to `viz_forest()` on the forest column | `tabviz()` |
| `tabviz(zoom=, auto_fit=, max_width=, max_height=, show_zoom_controls=)` | Pipe to `set_zoom()` after constructing the widget | `tabviz()` |
| `tabviz(weight = "col")` | `marker_size = "col"` | `tabviz()` |

`forest_plot()` still accepts `axis_range`, `axis_ticks`, and `axis_gridlines` natively — those were only deprecated on `tabviz()`.
