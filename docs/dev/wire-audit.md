...
| **B2** | Group padding grows huge after any widget change | density / row_kinds @ stage 7 | CLOSED 2026-06-05 (4d4c281 + 647faa1): the measure-then-commit loop committed `scrollHeight` of NON-overflowing cells — which echoes the pinned grid track back, pad included — so `row-padded-after` tracks re-added rowGroupPadding every frame (~12px/frame unbounded; hero hit 6,284px). Fixed with overflow-only commits + grow-merge; grouped-fixture regression gate in measure-rows.browser.ts proven red-on-prefix-bundle. Postmortem: sizing-model.md §6c. | **closed** |
| **B3** | Studio crashes on color change | anchor:paper / anchor:brand @ stage 5 or 7 | CLOSED-UNREPRODUCIBLE 2026-06-05: 45 scripted mutations across all 15 LCH sliders + Match-brand + polarity toggles on the live studio — no crash, no resolve-error banner, zero page errors. The crash path did not survive the wire-audit arc's rebuilds (resolver table, validateThemeInputs, LCH controls replacing the old input path); the Pass-4 keep-last-good boundary (`studio-store.resolveOutcome`) backstops any recurrence as a degraded banner instead of a white screen. | **closed** |
