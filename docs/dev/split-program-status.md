...
| Phase 3 — Publish | 🟡 **prep substantially done** | **`tsc --noEmit` gate: CLEAN (0 errors, down from 297)**. README written ✓. Toolchain canonical decision ✓. Remaining: CSS deliverable (separate ESM build with standalone style.css), V8 bundle-size CI gate, actual npm publish CI, the @ts-nocheck'd v1 fixture files (theme-presets, themes/default, swatches.test) want eventual v2 rewrite or ThemeSwitcher rewire. |
