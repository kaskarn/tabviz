...
| 11 | **C1** — forestStore decomposition (~4-5 weeks; long pole) | 🟡 in progress (9 of 11 slices). source (Q8) + cells + theme + axis + sort-filter + rows-groups + semantics + **drag + history micro-slices (0c-PR19)**. 124 vitest unit tests. Main store has shed ~1600 lines net. Three slices remain: columns, data, layout-zoom (long pole). |
