...
| Phase 3 — Publish | 🟡 **prep underway** | tsc-clean gate: **17 errors remaining** (down from 297, 280 closed cumulative). svg-generator is type-clean. Remaining 17: 12 splitForestStore (SplitSubviewOverride narrowing — independent of theme work), 2 forestStore, 3 swatches. Non-blocked prep items: **README (✓ written)**, **toolchain canonical decision (✓ npm canonical)**. Still pending: CSS deliverable (separate ESM build with standalone style.css), V8 bundle-size CI gate, actual npm publish CI. |
