...
| 11 | **C1** — forestStore decomposition (~4-5 weeks; long pole) | 🟡 in progress (10 of 11 slices). source (Q8) + cells + theme + axis + sort-filter + rows-groups + semantics + drag + history micro-slices + **columns (0c-PR20)**. 148 vitest unit tests. Main store has shed ~2000 lines net. Two slices remain: data, layout-zoom (long pole). |
