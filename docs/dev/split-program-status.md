...
| Phase 3 — Publish | 🟡 **prep underway** | tsc-clean gate: down to 257 errors (40 closed cumulative this iteration); ~212 remaining are v1→v2 theme drift, all C5-gated. Non-blocked prep items: **README (✓ written)**, **toolchain canonical decision (✓ npm canonical, recorded in README)**. Still pending: CSS deliverable (separate ESM build with standalone style.css), V8 bundle-size CI gate, actual npm publish CI. |
