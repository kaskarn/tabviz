...
| Phase 3 — Publish | 🟡 **prep underway; tsc-clean gate blocked on C5** | First `tsc --noEmit` ran today: 297 errors, 23 closed cheaply, **274 remain — ~212 of which are v1→v2 theme schema drift in `export/svg-generator.ts`, `lib/theme-presets.ts`, `themes/default.ts` (all gated on C5 theme-resolver port, deferred to Phase 1.x).** Other prep items NOT blocked: CSS deliverable (separate ESM bundle with `style.css`), V8 bundle-size CI gate, toolchain canonicalization (bun.lock vs package-lock.json), README. |
