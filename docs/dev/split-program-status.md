...
| Phase 3 — Publish | 🟡 **prep done; awaits maintainer-triggered publish** | All publish-gate scripts in place: `tsc --noEmit` CLEAN ✓, README ✓, toolchain canonical ✓, npm ESM build ✓ (`build:npm` → `dist/`), .d.ts types ✓, path-alias rewrite ✓, **bundle-size CI gate** ✓ (`check:size`), **lockfile-agreement CI gate** ✓ (`check:lockfiles`). What's left is a maintainer decision: version bump, name swap (`tabviz-js` → `@tabviz/core`), `npm publish`. The @ts-nocheck'd v1 fixture files (theme-presets, themes/default, swatches.test) want eventual v2 rewrite or ThemeSwitcher rewire, but they don't block first publish. |
...
| 11 | **C1** — forestStore decomposition (~4-5 weeks; long pole) | 🟡 in progress (5 of 11 slices). source (Q8) + cells (0c-PR13) + theme (0c-PR14) + axis (0c-PR15 — cross-slice $derived spike PASSED) + **sort-filter (0c-PR16)**. 65 vitest unit tests across slices; bun:test baseline unchanged. Plus `$lib/filter-sort-utils.ts` extracted (~225 lines of pure helpers). Main store has shed ~750 lines net. Next: rows-groups. |
