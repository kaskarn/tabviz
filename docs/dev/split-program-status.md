...
| 11 | **C1** — forestStore decomposition (~4-5 weeks; long pole) | 🟡 in progress (7 of 11 slices). source (Q8) + cells (0c-PR13) + theme (0c-PR14) + axis (0c-PR15) + sort-filter (0c-PR16) + rows-groups (0c-PR17) + **semantics (0c-PR18)**. 103 vitest unit tests across slices. Two levels of slice-to-slice $derived chain now in production. Main store has shed ~1500 lines net. Next: drag micro-slice. |
