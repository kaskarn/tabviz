...
| 11 | **C1** — forestStore decomposition (~4-5 weeks; long pole) | 🟡 in progress (4 of 11 slices). source (0c-PR0 / 23e18b4) + cells (0c-PR13) + theme (0c-PR14) + **axis (0c-PR15 — cross-slice $derived spike PASSED)**. 44 vitest unit tests across slices. All gates green. **The C1 idiom is fully validated**: `$derived` chains compose across slice boundaries via forward-closure getters with no ceremony. Next: sort-filter. |
