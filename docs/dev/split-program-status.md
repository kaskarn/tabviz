...
| 11 | **C1** — forestStore decomposition (~4-5 weeks; long pole) | 🟡 in progress (11 of 12 slices). source (Q8) + cells + theme + axis + sort-filter + rows-groups + semantics + drag + history micro-slices + columns + **data (0c-PR21)**. 175 vitest unit tests. Main store has shed ~2200 lines net. One slice remains: layout-zoom (the long pole). |
