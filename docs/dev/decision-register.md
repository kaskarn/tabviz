...
|----|----------|-----------|------|
| D13 | enable_themes roster: MEASURED (the register's ask) — the resolved 9-preset roster rides interaction.enableThemes at 43.9kB ≈ 88.8% of a small widget's payload. DECIDED: switch to INPUTS-form wire envelopes resolved in-widget (~10× smaller; the widget ships the full resolver). Implementation queued as a contained M3 arc (serializer emits envelopes; ThemeSwitcher builds via buildTheme on pick) | UNILATERAL (delegated): the measurement makes keep untenable and lazy-load needless — inputs-form is strictly better on every axis | 2026-06-11 |
